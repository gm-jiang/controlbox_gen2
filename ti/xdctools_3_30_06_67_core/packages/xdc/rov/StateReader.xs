...
function fetchModuleState(mod)
{
    ...
    mod.addr = $addr(symTable.getSymbolValue(mod.name.replace(/\./g, '_') +
                                             '_Module__state__V'));
    /* It's not clear yet what's are the conditions under which 'Module_State'
    ...
    if ('$constructInst' in Program) {
        ...
        for each(var constructInst in Program.$constructInst) {
            if (modStructType == constructInst.type) {
                var constructInstAddr = symTable.getSymbolValue(constructInst.name);
                if (constructInstAddr != -1) {
                    mod.dynInstAddrs.$add(constructInstAddr);
                }
            }
        }
    }
...
/*
 *  @(#) xdc.rov; 1, 0, 1,0; 1-20-2015 11:18:04; /db/ztree/library/trees/xdc/xdc-A65/src/packages/
 */
