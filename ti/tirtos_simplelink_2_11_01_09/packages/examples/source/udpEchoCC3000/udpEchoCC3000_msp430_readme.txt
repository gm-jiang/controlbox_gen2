Example Summary
----------------
Application that uses the WiFi driver and CC3000 device to echo UDP packets.
...
executable is found in:
  ...
  Example:
        udpSendReceive 192.168.1.100 1000 1 -l256 -s100
...
* Note: a SYS/BIOS Task is statically created in udpEchoCC3000.cfg and the
  RX and TX payload sizes are configured to 256 bytes.  Due to memory
  limits, the payload is kept lower than the 1460 byte maximum payload size.

  'udpEchoTask' performs the following actions:
