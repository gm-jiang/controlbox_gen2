/**************************************************************************************************
  ...
  Revised:        $Date: 2015-08-13 15:21:16 -0700 (Thu, 13 Aug 2015) $
  Revision:       $Revision: 44450 $
...
{
  if (macRxFilter != RX_FILTER_NON_COMMAND_FRAMES)
  {
    /* Restore the pan ID (passive and active scans set pan ID to 0xFFFF)
     * Do not restore the pan ID for orphan scan as it will create pan ID/
     * short address mismatch in upper layer. Note that the macRxFilter is
     * used to determine the scanMode. It is safe to do so since the
     * macRxFilter is modified only in this module.
     */
