...
static void Board_keyChangeHandler(UArg a0)
{
    ...
    if(appKeyChangeHandler != NULL)
    {
        ...
        else if(keysPressed == KEY_RIGHT)
        {
          ...
          if((rightDuration >= FACTORY_RESET_PRESS_PERIOD))
          {
            // Indicate to application that a reset and NV erase
            // should happen by saying that all the keys but the 
            // LEFT have been pressed, even tho the SensorTag only has
            // the left and right keys defined.
            keysPressed = (KEY_SELECT | KEY_UP |
                           KEY_DOWN | KEY_RIGHT);
          }
