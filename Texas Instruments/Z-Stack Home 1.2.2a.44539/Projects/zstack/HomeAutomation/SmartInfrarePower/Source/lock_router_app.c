...
uint16 lock_router_app_event_loop( uint8 task_id, uint16 events )
{
    ...
    {
        if (dev_meta_report_cn == REPORT_DEV_META_MAX_NUM) //dev_meta_report_cn , REPORT_DEV_META_MAX_NUM
        {
            log_printf( "report dev meta timeout, rejoin network.\r\n");
            SystemResetSoft();
            dev_meta_report_cn = 0;
        }
        else
        {
        ...
        return ( events ^ EVENT_REPORT_DEV_META );
    }
...
void lock_router_period_msg(void)
{
    ...
    send_msg_to_center((uint8 *)&msg_P, sizeof(router_period_msg_t), MSG_TYPE_POWER_PERIOD, msg_tid);
    if (ret != ZSuccess)
    {
        log_printf( "Error: report router period report, error code is %d.\r\n", ret);
        return ;
    }
    osal_start_timerEx(lock_router_app_task_id, LOCK_ROUTER_EVENT_PERIOD_MSG, (uint32)60*60*1000);
    return ;
}
...
void lock_router_msg_proc(afIncomingMSGPacket_t *msg)
{
    ...
    switch (type)
    {
       ...
       case MSG_TYPE_INFRARE:
            /*stop report*/
            log_printf("report infrare acked.\r\n");
            infrare_report_success();

            break;

       case MSG_TYPE_POWERID_REPORT:  //MSG_TYPE_DEV_META
            /*stop report*/
            log_printf("report dev meta acked.\r\n");
            osal_stop_timerEx(lock_router_app_task_id, EVENT_REPORT_DEV_META);
            report_dev_meta_finish();
