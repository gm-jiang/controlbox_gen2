...
static void YmodemSendByte (uint8 data)
{
  ...
  //sprintf(buf, "YmodemSendByte = %x\n", data);
  //YmodemOutputString(buf);
...
static int32 YmodemDownloading (void)
{
  ...
  while (1)
  {
    ...
    ch = 0;
    len = YmodemReceiveBytes(&ch, 1, PACKET_TIMEOUT);
    ...
    switch (ch)
    {
      ...
      case STX:
        ...
        *packet_data = ch;
        ...
        /*receive a packet*/
        len = YmodemReceiveBytes(packet_data + 1, packet_size + PACKET_OVERHEAD - 1, PACKET_TIMEOUT);
        if (len != packet_size + PACKET_OVERHEAD - 1)
        {
          YmodemSendByte(CRC16);
          errors++;
          break ;
...
void YModemUpgrade(void)
{
  ...
  if(YmodemReceiveBytes(cmd_data, 3, ENTER_UPGRADE_TIMEOUT) == 3)
  {
    if(cmd_data[0]==0x41 && cmd_data[1]==0x35 && cmd_data[2]==0x31)
    {
        YModemStartDownload ();
    }
  }
